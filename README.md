...
A regular radioactivity detector has only one detecting element so it gives only one measured value at a time. If multiple of detectors will be assembled to form a grid, it would be possible to measure objects (that fit to the detector area, here roughly 5x5 cm) and create images showing radioactive spots.

It would work a bit like a scanner for documents, but it wouldn't detect the ink position/shape, but radioactivity spots on objects.

To increase resolution, I think it would be possible to use a [super-resolution algorithm, that I implemented years ago](https://github.com/RobertGawron/supper-resolution), however, it needs that sample images are taken with small x-y offset of the image detector - that's the reason why the sensor is moving in x-y axis. The algorithm is implemented in Python and requires a lot of computations, so it will be run on Raspberry.
